...
test( "sha384", function () {
	...
	], 1, [] ).forEach( functools.partial( functools.star, function ( sha384name, sha384, string, expected ) {

		var digest;

		digest = sha384( ascii( string ), string.length * 8, array.alloc( 48 ) );

		deepEqual( digest, expected, sha384name + " " + string);
