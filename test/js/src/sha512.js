...
test( "sha512", function () {
	...
	], 1, [] ).forEach( functools.partial( functools.star, function ( sha512name, sha512, string, expected ) {

		var digest;

		digest = sha512( ascii( string ), string.length * 8, array.alloc( 64 ) );

		deepEqual( digest, expected, sha512name + " " + string);
