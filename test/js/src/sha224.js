...
test( "sha224", function () {
	...
	], 1, [] ).forEach( functools.partial( functools.star, function ( sha224name, sha224, string, expected ) {

		var digest;

		digest = sha224( ascii( string ), string.length * 8, array.alloc( 28 ) );

		deepEqual( digest, expected, sha224name + " " + string);
